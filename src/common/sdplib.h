...
typedef struct _fmtp_attr
{
	...
	int sps_size;
	unsigned char sps[200];
	int pps_size;
	unsigned char pps[200];
}FmtpAttr_t;
...
typedef struct _attrubute
{
	...
	union{
		...
		RtpMapAttr_t rtpmap;
		float framerate;
		int framesize; // width[MSB 16] * height [LSB 16]
	};
}Attribute_t;
...
extern int SDP_add_sps_pps(SessionDesc_t *sdp, unsigned char *sps, int sps_size, 
	unsigned char *pps, int pps_size);
...
extern int SDP_get_h264_info(SessionDesc_t *sdp,int *payloadtype,char *ip,int *port, 
	void *sps,int *sps_size, void *pps, int *pps_size, 
	int *framerate, int *width, int *height);
extern int SDP_get_g711_info(SessionDesc_t *sdp,int *payloadtype,char *ip,int *port);
